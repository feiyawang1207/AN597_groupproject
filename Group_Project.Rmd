...
methodological approaches that allow generating useful information from incomplete data; such is the case of the modeling of the ecological niches of the species and their geographical distributions.

What is the geographical distribution of a specie?
In the simplest definition, it is its spatial arrangement on the planet. It is the geographical description of where it lives.

Traditionally, the distribution of the species has been represented in a cartographic way from the geo-referenced points on maps. Without considering the local factors that determine it.

SDMs are correlative methods, static and non mechanistic, that work by identifying the relationships that exist between the conditions of the environment and the sites of presence of the species.

To make the best use of SDMs it is very important know how it works, but equally important is it is necessary to know the limitations of the data sources, algorithms and the methodological approach in general.
...
In this module we provide a short summary to species distribution modelling in R. This module will cover a brief overview of the concept of species distribution modelling and the basic modelling steps. 
...
Notes: Before you begin you will need to install the following packages and their dependencies:
...
Follow a brief description of the main packages that we will use in this module: dismo (="distribution modeling"), and raster (="geographic data analysis and modelling"), and each one of them has a set of functions that enable to modelling species distribution in geographic and spatial contexts
...
To importing data from a database, we use a function called `gbif ()` to download the files from [GBIF](https://www.gbif.org) database. 

We are going to download the occurrence data from a potato species. *Solanum acaule* is widespread and common in upland habitats from northern Peru (Dept. Cajamarca), south through Bolivia to northern Argentina (Prov. San Juan), and with one record in northern Chile (Antofagasta Region)
...
First, we need to draw a empty map. a variable called [wrld_simpl!](https://www.rdocumentation.org/packages/maptools/versions/0.9-8/topics/wrld_simpl) will show a empty world-wide map. The wrld_simpl dataset contains rough country outlines.
...
We imported more than seven thousand records, but not all of these are usable because of various issues. This species is distributed mainly in the Andes mountains of southern Peru, Bolivia and northern Argentina, but as you can see in the previous map there are records in other countries and even in the Antartica. In order to clean this data we will need to: 
...
The records might come from different sources, sometimes from museum collections or zoos. Be sure to choose only the kind of records you really want
```{r}
...
Many databases share data, so you risk including duplicate records. The next function will help you to remove duplicates.
...
In the example below we use the coordinates function from the `sp` package to create a spatial points data frame, and then the over function, also from `sp`, to do a point-in-polygon query with the countries polygons.
...
Bioclim, Domain, and Mahal are methods from profile mothods.These methods are implemented in the dismo package, and the procedures to use these models are the same for all three.

>Regression model:

We lean classic regression model from class. it is very similar here. Models are fit using maximum likelihood and by allowing the model to be related to the response variable via a link function and by allowing the magnitude of the variance of each measurement to be a function of its predicted value 
